...
var SetupModal = class extends Modal {
  ...
  onOpen() {
    ...
    const schema = Object.assign({}, DEFAULT_SETTINGS.schema, settings.schema || {});
    const fields = [
      ...
      ["任务类型字段", "typeField", schema.typeField],
      ["任务类型值", "typeValue", schema.typeValue],
      ["状态字段", "statusField", schema.statusField],
      ["计划日期字段", "planField", schema.planField],
      ["所属项目字段", "projectField", schema.projectField],
      ["优先级字段", "priorityField", schema.priorityField]
    ];
    ...
    save.addEventListener("click", async () => {
      ...
      const nextSchema = Object.assign({}, schema);
      ["typeField", "typeValue", "statusField", "planField", "projectField", "priorityField"].forEach((key) => nextSchema[key] = inputs.get(key).value.trim() || schema[key]);
...
var FocusWorkbenchView = class extends ItemView {
  ...
  allTaskFiles() {
    ...
    return this.files().filter((file) => this.starts(file, taskFolder) && String(this.meta(file)[schema.typeField] || "") === schema.typeValue);
  }
  projectOptions() {
  ...
  renderIdeaQueue(parent, title, description, files, overdue, collapsed, toggle) {
    ...
    if (!files.length) {
      const empty = list.createDiv({ cls: "pvd-triage-empty" });
  ...
  renderArticleResults(parent, groups = this.knowledgeGroups()) {
    ...
    visibleGroups.forEach((group) => {
      ...
      const list = section.createDiv({ cls: "pvd-article-list" });
      notes.slice(0, 60).forEach((file) => {
        const article = this.button(list, "", () => this.openFile(file), "pvd-article");
  ...
  renderTaskCalendar(shell, tasks) {
    ...
    for (let day = 1; day <= days; day += 1) {
      ...
      dayTasks.slice(0, 3).forEach((file) => {
        const chip = this.button(list, file.basename, () => this.setFocus(file), `pvd-calendar-task ${this.taskDone(file) ? "is-done" : ""} ${this.priority(file).toLowerCase()}`);
  ...
  renderCompletedTasks(parent, tasks) {
    ...
    if (this.completedExpanded) {
      const list = section.createDiv({ cls: "pvd-completed-list" });
      completed.forEach((file) => this.renderTaskCard(list, file));
    }
  }
...
`);
  ...
  async convertIdeaToNote(file, kind) {
    const linkedTask = this.ideaLinkedTask(file);
    const previousPath = file.path;
    const target = kind === "literature" ? { dir: this.config().literature, type: "文献笔记", status: "待整理", notice: "已转为文献笔记" } : { dir: this.config().permanent, type: "永久笔记", status: "已沉淀", notice: "已转为永久笔记" };
    ...
    const path = this.uniqueMovePath(target.dir, file);
...
var FocusWorkbenchSettingTab = class extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(hero).setName(readyCount === 3 ? "推荐结构已准备好" : "自动创建推荐结构").setDesc("包括 3 个知识目录、任务、项目与每日进展目录，以及任务模板和 Obsidian Bases 任务总表；可以重复执行，已有内容不会被改写。").addButton((button) => button.setButtonText(readyCount === 3 ? "检查并补齐" : "一键初始化").setCta().onClick(() => new ConfirmModal(this.app, "初始化推荐工作区", "将补齐三类知识文件夹、任务与项目目录、任务模板和任务总表。已有文件不会被覆盖，是否继续？", "开始初始化", async () => {
      var _a, _b;
    ...
    new Setting(templateCard).setName(templateReady ? "模板已找到" : "等待初始化").setDesc(value("taskTemplatePath")).addText((text) => text.setValue(value("taskTemplatePath")).setPlaceholder(defaults.taskTemplatePath).onChange(async (next) => {
